  ...
  return (
    <div className="min-h-screen bg-slate-950 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="flex-shrink-0 h-14 flex items-center gap-3 px-4 lg:px-6 sticky top-0 z-30"
          style={{ background: "rgba(8,13,26,0.94)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <button onClick={() => setMobileOpen(true)} className="lg:hidden text-slate-600 hover:text-slate-300 p-1.5 rounded-xl hover:bg-slate-800/60 transition-all">
            <Menu size={19} />
          </button>
          ...
          <div className="flex items-center gap-2">
            {client.kycStatus === "VERIFIED" && (
              <span className="hidden sm:flex items-center gap-1 text-[10px] px-2.5 py-1 rounded-full font-bold"
                style={{ background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.25)", color: "#34d399" }}>
                <Shield size={9} /> Verified
              </span>
            )}
            <Link to="/portal/notifications" className="relative text-slate-600 hover:text-slate-300 p-1.5 rounded-xl hover:bg-slate-800/60 transition-all">
              <Bell size={18} />
              {notifCount > 0 && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-red-500 rounded-full text-[8px] text-white flex items-center justify-center font-bold shadow-lg">
                  {notifCount > 9 ? "9+" : notifCount}
                </span>
              )}
            </Link>
            <Link to="/portal/profile"
              className="w-8 h-8 rounded-full flex items-center justify-center font-black text-xs shadow-lg transition-all hover:scale-105"
              style={{ background: "linear-gradient(135deg,#C9A227,#F5A623)", color: "#0B1F3A" }}>
              {initials}
            </Link>
          </div>
        </header>
        ...
        {/* Page */}
        <main className="flex-1 overflow-y-auto p-4 lg:p-6 pb-24 lg:pb-6 portal-scroll portal-page-enter">
          <Outlet />
        </main>
        ...
        {/* Mobile bottom nav — premium gold-accent style */}
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 px-3 pb-safe"
          style={{ background: "rgba(8,13,26,0.97)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-end justify-around py-2">
            {bottomNav.map(item => {
              const isActive = location.pathname === item.href;
              return (
                <NavLink key={item.href} to={item.href}
                  className="flex flex-col items-center gap-1 relative px-2 py-1 min-w-[52px]">
                  {item.highlight ? (
                    <div className="relative">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-xl transition-all -mt-5 ${isActive ? "shadow-amber-900/50" : "shadow-amber-900/30"}`}
                        style={{ background: isActive ? "linear-gradient(135deg,#C9A227,#F5A623)" : "linear-gradient(135deg,#b8911f,#C9A227)" }}>
                        <item.icon size={22} className="text-[#0B1F3A]" />
                      </div>
                    </div>
                  ) : (
                    <div className="relative w-8 h-8 flex items-center justify-center">
                      <item.icon size={20} className={`transition-colors ${isActive ? "text-[#C9A227]" : "text-slate-600"}`} />
                      ...
                      {isActive && !item.highlight && (
                        <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#C9A227]" />
                      )}
                    </div>
                  )}
                  <span className={`text-[9px] font-semibold transition-colors ${isActive ? (item.highlight ? "text-[#C9A227]" : "text-[#C9A227]") : "text-slate-700"} ${item.highlight ? "mt-1" : ""}`}>
                    {item.label}
                  </span>
                </NavLink>
              );
